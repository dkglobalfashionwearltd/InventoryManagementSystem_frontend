...
export default function ComboboxItemUser({
  options,
  value = null,
  onChange,
  placeholder = "Select...",
  ...
  useEffect(() => {
    ...
    const filtered = options
      .filter((o) => {
        const name = o.name ?? "";
        const officeId = o.officeId.toString() ?? "";
        return (
          name.toLowerCase().includes(q) || officeId.toLowerCase().includes(q)
        );
      })
      .slice(0, maxResults);
        ...
        <input
          ...
          aria-activedescendant={
            highlightedIndex >= 0 && filtered[highlightedIndex]
              ? `cb-item-${id}-${filtered[highlightedIndex].itemUserId}`
              : undefined
          }
          className={
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((opt, idx) => {
                ...
                return (
                  <li
                    key={opt.itemUserId}
                    id={`cb-item-${id}-${opt.itemUserId}`}
                    role="option"
                    aria-selected={value?.itemUserId === opt.itemUserId}
                  ...
                  >
                    ...
                    {value?.itemUserId === opt.itemUserId && (
                      <svg
                        className="h-4 w-4"
                        viewBox="0 0 20 20"
                        fill="none"
